...
export function NordLayerSettingsForm({
  hasCredentials,
  saveAction,
  testAction,
}: {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      <div>
        <h2 className="text-sm font-semibold text-slate-900">NordLayer</h2>
      ...
      <form action={formAction} className="mt-4 space-y-3">
        <div>
          ...
          <input
            type="password"
            name="api_key"
            placeholder={hasCredentials ? "Leave blank to keep the current key" : "Paste the API key (msp_...)"}
            autoComplete="off"
      ...
      <p className="mt-4 text-xs text-slate-500">
        Generate a key under Integrations in the NordLayer Service Management Portal (SMP). This
        just confirms the connection for now — a per-client organization mapping and lookups
        (usage reports, plan details) are a follow-up, not wired up yet.
      </p>
    </div>
  );
}
