...
export function HuntressSettingsForm({
  hasCredentials,
  saveAction,
  testAction,
}: {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <form action={formAction} className="mt-4 space-y-3">
        ...
        <div>
          <label className="block text-xs font-medium text-slate-700">API secret</label>
          <input
            type="password"
            name="api_secret"
            placeholder={hasCredentials ? "Leave blank to keep the current secret" : "Paste the API secret"}
            autoComplete="off"
            className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
          />
        </div>
      ...
      <p className="mt-4 text-xs text-slate-500">
        Generate a key/secret pair under Settings → API Credentials in the Huntress dashboard —
        the full secret is shown only once, so save it somewhere safe when you create it. This
        just confirms the connection for now; a per-client organization mapping and lookups (agent
        health, incident reports) are a follow-up, not wired up yet.
      </p>
    </div>
  );
}
