...
export function HuduSettingsForm({
  hasCredentials,
  currentBaseUrl,
  saveAction,
  testAction,
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      <div>
        <h2 className="text-sm font-semibold text-slate-900">Hudu</h2>
      ...
      <form action={formAction} className="mt-4 space-y-3">
        <div>
          <label className="block text-xs font-medium text-slate-700">Hudu URL</label>
          <input
            name="base_url"
            defaultValue={currentBaseUrl ?? ""}
            placeholder="https://yourcompany.hudu.com"
            autoComplete="off"
            className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
          />
          <p className="mt-1 text-xs text-slate-400">
            The domain you log into your Hudu instance with.
          </p>
        </div>
        <div>
      ...
      <p className="mt-4 text-xs text-slate-500">
        Generate a key under Admin → API Keys in your Hudu instance. This just confirms the
        connection for now — pulling client credentials (e.g. M365 app registrations) out of Hudu
        is a follow-up, not wired up yet.
      </p>
    </div>
  );
}
