    ...
    results: {
      ...
      items: {
        ...
        properties: {
          ...
          job_stability_reasoning: {
            type: ["string", "null"],
            description:
              "REQUIRED working step before job_stability below — list the 2-3 MOST RECENTLY HELD roles, most recent first, each as \"Employer: N months\" (or years), with the duration actually calculated from each role's own stated start/end dates. E.g. \"HCLTech: 3 months; Toronto Parking Authority: 4 months; Elections Ontario: 4 months.\" Do the arithmetic explicitly rather than eyeballing it — this is what job_stability below must be based on. Null only if there's no dated work history at all to compute from.",
          },
          job_stability: {
            type: ["string", "null"],
            enum: ["stable", "frequent_changes", null],
            description:
              "Based on the durations just listed in job_stability_reasoning above (review EVERY employer/position, not just the most recent one, weighing RECENT roles more heavily than older ones — a stable job from years ago does NOT offset a recent pattern of short stints): \"frequent_changes\" if the two or three MOST RECENTLY COMPLETED roles were each under about 2 years, OR there's a broader pattern of 3+ short stints (each under about 1-2 years) anywhere in the history. Their current, still-ongoing role doesn't count against this on its own (it hasn't ended yet), but a string of short roles immediately before it still does. \"stable\" only when the durations above don't show that pattern, or there's too little work history to judge from at all. Null only if no dates/work history are given to judge from.",
          },
          last_job_in_canada: {
            type: ["boolean", "null"],
            description:
...
Exchange Admin"; not just end-user app use). Before judging job_stability, fill in
job_stability_reasoning first: list the 2-3 most recently held roles, most recent first,
each as "Employer: N months" (or years) with the duration actually calculated from that
role's own stated dates — do the arithmetic, don't eyeball it. Then set job_stability from
those durations: review EVERY employer listed, not just the most recent one, weighing
RECENT roles more heavily than older ones (a stable job from years ago does not offset a
recent pattern of short stints): "frequent_changes" if the two or three MOST RECENTLY
COMPLETED roles were each under about 2 years, or there's a broader pattern of 3+ short
stints (each under about 1-2 years) anywhere in the history (their current, still-ongoing
role doesn't count against this on its own, but a string of short roles immediately before
it still does); "stable" only when the durations above don't show that pattern, or there's
too little work history to judge from — and last_job_in_canada (is their MOST RECENT job's stated
  ...
  for (let i = 0; i < rows.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      type ScreeningResult = {
        ...
        job_stability_reasoning?: string | null;
      ...
      for (let n = 0; n < chunk.length; n++) {
        ...
        const comment = [
          ...
          result.customer_relationships
            ...
            : null,
          result.job_stability_reasoning ? `Job stability: ${result.job_stability_reasoning}` : null,
        ]
          .filter(Boolean)
          .join("\n\n");
